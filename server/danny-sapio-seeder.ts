...
export async function seedDannySapioStrategy() {
  ...
  const modules = [
    ...
    },
    {
      ...
      notesPrompt: "Document your criteria for identifying market opportunities and your increased buying plan during downturns. Record your psychological preparation strategies.",
      resources: {
    ...
    },
    {
      ...
      notesPrompt: "Map your current portfolio allocation by sector and individual position sizes. Plan your rebalancing targets and schedule.",
      resources: {
    ...
    },
    {
      ...
      notesPrompt: "Calculate your 10, 20, and 30-year wealth projections based on current investment amounts. Document your commitment strategies for long-term discipline.",
      resources: {
    ...
    },
    {
      ...
      notesPrompt: "Document your performance tracking system, optimization discoveries, and scaling plans. Record your long-term wealth preservation strategy.",
      resources: {
  ...
  // Insert all modules
  console.log(`🌱 Inserting ${modules.length} modules for Danny Sapio's strategy...`);
  
  for (let i = 0; i < modules.length; i++) {
    const module = modules[i];
    try {
      await db.insert(trainingModules).values(module);
      console.log(`✅ Inserted module ${i + 1}/${modules.length}: ${module.slug}`);
    } catch (error) {
      console.error(`❌ Failed to insert module ${module.slug}:`, error);
      throw error;
    }
  }

  console.log("✅ Danny Sapio's Robinhood Strategy modules seeded successfully");
}