...
async function buildAuctionMarketTheoryOnly() {
  console.log("🎯 Building Auction Market Theory strategy...");
  ...
  // Import and run just the auction market theory build function
  const trackId = "auction-market-theory";
  await db.delete(trainingModules).where(eq(trainingModules.trackId, trackId));
  
  // Import the build function from comprehensive-strategy-builder
  const module = await import("./comprehensive-strategy-builder");
  
  // Call build function using eval to access the private function
  // Actually, let's just re-import and duplicate the building code
  const modules = [
    {
  ...
  console.log("✅ Auction Market Theory strategy built (1/8 modules - test module only)");
  console.log("🎉 Complete! Ready to view in UI.");
  process.exit(0);
}

buildAuctionMarketTheoryOnly().catch(err => {
